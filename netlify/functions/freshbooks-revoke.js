...
exports.handler = async (event) => {
  ...
  try {
    ...
    const connRows = await sb(`freshbooks_connections?business_id=eq.${enc(business_id)}&select=refresh_token`);
    ...
    await sb(`freshbooks_connections?business_id=eq.${enc(business_id)}`, { method: 'DELETE' });
...
function revoke(refresh_token) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on('end', () => {
        if (res.statusCode >= 400) console.warn(`FreshBooks revoke returned ${res.statusCode}: ${data}`);
        else console.log('FreshBooks token revoked successfully');
        resolve();
      });
    });
    req.on('error', reject);
